...
Zisb defines a hardware protection domain that consists of code and data
...
* The current address space identifier (ASID), when the supervisor mode is also
  implemented and the `satp.MODE` is not `Bare`.
* When the H extension is also implemented:

  ** The current virtualization mode.
  ** The current virtual machine identifier (VMID), when the `hgatp.MODE` is not
     `Bare`.

* When the Smsdid extension is also implemented:
  ** The current supervisor domain identifier (SDID)

When the Zisb extension is implemented, predictions learned in one hardware
protection domain cannot be used to predict outcomes in another hardware
protection domain.

When an identifier used to identify a hardware protection domain is reused,
software must execute fence.time to ensure that predictions created by code that
previously executed in that hardware protection domain do not affect outcomes
for code that subsequently executes in that hardware protection domain.

[NOTE]
====
When Zisb is implemented, the predictors for control-flow, data-dependency, and
data-value prediction may be tagged with the identifiers of the hardware
...
domain changes. A combination of these two techniques may also be employed,
wherein some types of predictors use tagging and others use invalidation as a
defense.

Executing `fence.time` may incur substantial overhead. When Zisb is implemented,
`fence.time` is required only when an identifier is reused. If an implementation
provides a sufficiently large identifier space, reuse is infrequent, which
mitigates the cost of invoking the fence.
====

The Zisb extension provides three barrier instructions to control speculation
...
not expose data through side channels.
...
be exploited as a covert channel, allowing an attacker to infer sensitive
...
26. // information through a side channel. The MDB at line 29
