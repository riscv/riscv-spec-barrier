...
* A load from memory that appears in program order after MDB must obtain its
  data from a store to the same physical memory location that appears in program
  order before MDB. These semantics shall be enforced regardless of whether the
  load and/or the store are speculative, and regardless of whether the load
  and/or store eventually complete.
...
* A load from memory that precedes MDB in program order must not speculatively
  obtain its data from store to memory that appear in program order after MDB.
